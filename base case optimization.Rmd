...
ggplot(errors, aes(fantPts_bin, sd,colour=Pos)) + 
...
hist(projections$fantPts[projections$Pos=="WR"& projections$Season%in% 2012:2017& as.character(projections$fantPts_bin)=="(150,200]"]-
       projections$fantPts_agg[projections$Pos=="WR"& projections$Season%in% 2012:2017& as.character(projections$fantPts_bin)=="(150,200]"], 
     main="WR Actual-Projected, 150 to 200 FPTS projected", xlab="")
```
...
```{r  include=T, eval=T , echo=F}
topLineup<-simSeason(picks,numRB=2, numWR=2, numTE=1, numQB=1, numK=1, numDST=1, numFLEX=1, scoring="HALF", returnLineup = T)
topLineup
```
