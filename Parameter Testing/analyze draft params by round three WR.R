...
#shift=0
drafts<-lapply(1:numDrafts,function(x)simDraft(scoring=scoring,shift=0, numWR=6)) 
...
#shift=0, WR in R1
drafts4<-lapply(1:numDrafts,function(x)simDraft(scoring=scoring,shift=0, numWR=6,outPos = c(rep("RB", 1)))) #draft 1 qb in midrounds
simScores4<-foreach(x=drafts4, .packages = c("data.table", "dplyr", "plyr"))%dopar%{
  replicate(numSims, simSeason(x, scoring=scoring, numWR = 3)) }

#shift=0, zeroRB in R1-4sim
drafts5<-lapply(1:numDrafts,function(x)simDraft(scoring=scoring,shift=0, numWR=6,outPos = c(rep("WR", 1))))  #draft 1 qb in midrounds
...
#shift=0, heavyWR
drafts7<-lapply(1:numDrafts,function(x)simDraft(scoring=scoring,shift=0, numRB=4, numWR=7, numTE=1, numQB=2,numK=1, numFLEX=0, numDST=1))
...
drafts8<-lapply(1:numDrafts,function(x)simDraft(scoring=scoring,shift=0, numRB=6, numWR=5, numTE=1, numQB=2,numK=1, numFLEX=0, numDST=1)) 
simScores8<-foreach(x=drafts8, .packages = c("data.table", "dplyr", "plyr"))%dopar%{
  replicate(numSims, simSeason(x, scoring=scoring, numWR = 3)) }
quantile(unlist(simScores8))

#shift=0, no QB backup
drafts9<-lapply(1:numDrafts,function(x)simDraft(scoring=scoring,shift=0, numRB=5, numWR=6, numTE=2, numQB=1,numK=1, numFLEX=0, numDST=1))
simScores9<-foreach(x=drafts9, .packages = c("data.table", "dplyr", "plyr"))%dopar%{
  replicate(numSims, simSeason(x, scoring=scoring, numWR = 3)) }

#shift=0, QB+TE backup, <=1 QB in R1-11
drafts10<-lapply(1:numDrafts,function(x)simDraft(scoring=scoring,shift=0,numRB=4, numWR=6, numTE=2, numQB=2, numK=1, numDST=1 )) #onePos = rep("QB", 11)
simScores10<-foreach(x=drafts10, .packages = c("data.table", "dplyr", "plyr"))%dopar%{
  replicate(numSims, simSeason(x, scoring=scoring, numWR = 3)) }

#zero WR
drafts11<-lapply(1:numDrafts,function(x)simDraft(scoring=scoring,shift=0,numRB=5, numWR=6, numTE=1, numQB=2, numK=1, numDST=1,
...
drafts_allSlots<-lapply(paste("Slot", 1:12, sep=""), function(x)
  lapply(1:numDrafts,function(y) simDraft(scoring=scoring,shift=0,numRB=4, numWR=7, numTE=1, numQB=2, numK=1, numDST=1 , slot = x))
)
...
drafts_allSlots_zeroRB<-lapply(paste("Slot", 1:12, sep=""), function(x)
  lapply(1:numDrafts,function(y) simDraft(scoring=scoring,shift=0,numRB=4, numWR=7, numTE=1, numQB=2, numK=1, numDST=1 , slot = x, outPos=rep("RB", 1)))
)
...
drafts_allSlots_zeroWR<-lapply(paste("Slot", 1:12, sep=""), function(x)
  lapply(1:numDrafts,function(y) simDraft(scoring=scoring,shift=0,numRB=4, numWR=7, numTE=1, numQB=2, numK=1, numDST=1 , slot = x, outPos=rep("WR", 1)))
)
simScores_allSlots_zeroWR<-foreach(x=drafts_allSlots_zeroWR,.packages = c("data.table", "dplyr" ,"plyr"))%dopar%{
  lapply(x, function(y)replicate(numSims,simSeason(y, scoring=scoring, numWR=3)))}

save(list=ls()[grepl("simScores|drafts",ls())], file=paste0("Parameter Testing/" ,scoring," analyze draft params by round 3WR.RData"))
